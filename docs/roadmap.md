...
| 15 | 機能削減（2026-07-26）: 開始通知を `/start` の返信へ統合、通知フラグ2列を廃止 | ✅ 完了 |

実装は一巡し、ユニットテスト 233 件が通っている状態。**Discord / YouTube への実接続は未検証**（トークン・API キー・投稿先チャンネル ID の設定が必要）。
...
- [ ] `/start` の返信に開始時刻が入り、**開始の知らせがこの1通だけ**であること（重複していないか）
