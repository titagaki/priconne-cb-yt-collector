...
- まとめ動画（`is_summary = true`）は専用スレッド（または通常チャンネル）へ（[06](06-classification.md)）
...
フッター: {チャンネル名} ・ {投稿日時 JST} ・ フルオート
色: ボスごとに固定色（index で決め打ち）
```
...
- 429 を受けたら `Retry-After` に従って待機し、キューは維持する
